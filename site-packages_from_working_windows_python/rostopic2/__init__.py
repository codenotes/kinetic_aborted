...
def getAllTopics():
    args=['boob','list2']
    _rostopic_cmd_list2(args)
    return gTopics

def _sub_rostopic_list2(master, pubs, subs, publishers_only, subscribers_only, verbose, indent=''):
    global gTopics
    def topic_type(t, topic_types):
        matches = [t_type for t_name, t_type in topic_types if t_name == t]
        if matches:
...
def _rostopic_list2(topic, verbose=False,
                   subscribers_only=False, publishers_only=False,
                   group_by_host=False):
    """
    Print topics to screen
    ...
    if group_by_host:
        ...
        for hostname in set(list(host_pub_topics.keys()) + list(host_sub_topics.keys())): #py3k
            ...
            if (pubs and not subscribers_only) or (subs and not publishers_only):
                print("Host [%s]:" % hostname)
                _sub_rostopic_list2(master, pubs, subs,
                                   publishers_only, subscribers_only,
                                   verbose, indent='  ')
    else:
        _sub_rostopic_list2(master, pubs, subs,
...
def _rostopic_cmd_list2(argv):
    ...
    if options.bag:
        if options.subscribers: 
            ...
            parser.error("--host option is not valid with bags")
        _rostopic_list_bag2(options.bag, topic)
    else:
        if options.subscribers and options.publishers:
            parser.error("you may only specify one of -p, -s")

        exitval = _rostopic_list2(topic, verbose=options.verbose, subscribers_only=options.subscribers, publishers_only=options.publishers, group_by_host=options.hostname) or 0
        if exitval != 0:
            sys.exit(exitval)
