...
## but this is MY(greg) converter, and I am monkeying it with. make sure rospack.exe is built and in path for this to work
## but you can generate classes with it, so that's pretty cool
...
MSG_TYPE_TO_UNREAL = {'byte': 'int8', 'char': 'uint8_t',
                   'bool': 'bool',
                   ...
                   'string': 'FString',
                   'time': 'FDateTime',
                   'duration': 'Unreal::Duration?'} #todo

#remember all created fields so we can do assignments from Unreal types and back again
#UNREAL_ASSIGNMENT_FUNCTIONS= {
#    ('string','FString'):('  %s=TCHAR_TO_ANSI(*%sROS);\n  %sROS=ANSI_TO_TCHAR(%s);\n\n',4)
#    }
UNREAL_ASSIGNMENT_FUNCTIONS= {
    ('string','FString'):{'toROS':('%s=TCHAR_TO_ANSI(*%sROS);',2), 'fromROS':('%sROS=ANSI_TO_TCHAR(%s.c_str());',2) },
    ('int32','int32'):     {'toROS':('%s=%sROS;',2), 'fromROS':('%sROS=%s;',2) },
    ('int8','int8'):     {'toROS':('%s=%sROS;',2), 'fromROS':('%sROS=%s;',2) },
    ('int16','int16'):     {'toROS':('%s=%sROS;',2), 'fromROS':('%sROS=%s;',2) },
    ('uint8','uint8'):     {'toROS':('%s=%sROS;',2), 'fromROS':('%sROS=%s;',2) },
    ('uint8','uint8'):     {'toROS':('%s=%sROS;',2), 'fromROS':('%sROS=%s;',2) },
    ('uint32','uint32'):     {'toROS':('%s=%sROS;',2), 'fromROS':('%sROS=%s;',2) },
    ('uint64','uint64'):     {'toROS':('%s=%sROS;',2), 'fromROS':('%sROS=%s;',2) },
    ('float','float'):     {'toROS':('%s=%sROS;',2), 'fromROS':('%sROS=%s;',2) },
...
def write_member(s, field):
    ...
    #UNREAL_CLASS_INFO[field.name]=(field.type,MSG_TYPE_TO_UNREAL[field.type])


def write_unreal_class():
    toROS=[]
    fromROS=[]
    try:
        for k, v in UNREAL_CLASS_INFO.iteritems():
            print('  %s %sROS;\n\n'%(MSG_TYPE_TO_UNREAL[v[0]], k)) #greg2, testing, don't do this here
            d=ROStoUnrealConverterFunctions(v[0], MSG_TYPE_TO_UNREAL[v[0]],k)
            toROS.append(d['toROS'])
            fromROS.append( d['fromROS'])
    except e:
        print e
        return
...
def ROStoUnrealConverterFunctions(ros_type, unreal_type, fname):
# ('string','FString'):{'toROS':('%s=TCHAR_TO_ANSI(*%sROS);',2), 'fromROS':('%sROS=ANSI_TO_TCHAR(%s);',2) } //UNREAL_ASSIGNMENT_FUNCTIONS
    s=UNREAL_ASSIGNMENT_FUNCTIONS[(ros_type,unreal_type)] #matches 2 part key on UNREAL_ASSIGNMENT_FUNCTIONS
    d={ 'toROS':s['toROS'][0]%( tuple([fname]*  s['toROS'][1]  )), 'fromROS':s['fromROS'][0]%( tuple([fname]*  s['fromROS'][1]  )) }
    return d
    #return s[0] % ( tuple([fname]*s[1])) # s[1] is number of times fname should appear to satisfy the %s in the format string s[0].  



def write_assignments():

    #x={'field1':['string','FString']}
...
def generate(msg_path):
    ...
    s.write('UNREAL CLASS HEADER STUFF\n\n')
    s.write('namespace %s\n{\n'%(package))
...
#              C:\\ros\\kinetic\\share\\ROSPyTesting
   # SOMETHING IS DIFFERENT, former won't work with embedded stuff but other will
   # generate("C:\\ros\\kinetic\\share\\ROSPyTesting\\msg\\gregtestmsg.msg")
    generate("C:\\ros\\kinetic\\share\\geometry_msgs\\msg\\Accel.msg")
    #print UNREAL_CLASS_INFO
    #write_unreal_class()
...
#last run md5=fb8c9858d39971050e96baeabc29cc87
#added variable:bb81f539c0d5fac388175c6aa063be99
#removed variable:fb8c9858d39971050e96baeabc29cc87
#so...md5 is solely based on .msg form. Good to know. //greg2